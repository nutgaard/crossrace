...
export function findOrdering(racelog: string): RiderOrder[] {
  const allPasses: string[] = parseRacelog(racelog);
...
export function findUnrecognizedDrivers(racelogStr: string, racersStr: string): string[] {
  const racelog = parseRacelog(racelogStr);
  const racers = parseRacers(racersStr);

  const unrecognizedRacers = new Set<string>();
  for (const racer of racelog) {
    if (!(racer in racers)) unrecognizedRacers.add(racer);
  }

  return Array.from(unrecognizedRacers);
}

function parseRacelog(racelog: string): string[] {
  return racelog
    .replace(/\t/g, ' ')
    .trim()
    .split('\n')
